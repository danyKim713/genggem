						...
						<? 
							...
							while ($row = mysqli_fetch_array($resultCategory)) {
								...
								if (is_file($_SERVER[DOCUMENT_ROOT]."/ImgData/WatchCatImg/".$row['cat_img'])) { 
									$strImg = "<img src=\"/ImgData/WatchCatImg/{$row["cat_img"]}\" width=\"50\"  alt=\"{$row["cat_nm"]}\">";
								}
						?>

                            <li class='col-3 radiobox'>
                                <input id="type<?=$i?>" type="radio" name="rdoCat" class="rdoCat invisible" value="<?=$row["cat_id"]?>" />
                                <label for="type<?=$i?>">											
                                    <?=$strImg?>
...
?>
                ...
                <div class="form-group">
                    ...
                    <input  type="text" class="form-control" id="txtTitle" name="txtTitle" placeholder="영상제목을 입력해 주세요." />
                </div>
                <div class="form-group">
                    <label for="">영상태그 (쉼표로 구분해서 입력)</label>
                    <input type="text" class="form-control" id="txtTag" name="txtTag" placeholder="예)부킹,여행,프로교육" />
                </div>
                <div class="form-group">
                    <label for="">영상링크주소 (유투브 영상만 링크 가능)</label>
                    <input type="text" class="form-control" id="txtLink" name="txtLink" placeholder="예)https://youtu.be/Flnv5JN3Jng" />
                </div>
                <!-- <div class="form-group box">
                    <label for="">썸네일이미지</label>
                    <span class="fs--1 float-right fw-400 mt-1 color-6">미리보기 이미지를 업로드 해주세요</span>
                    <div class="js-image-preview" style="background-image:url()">
...
?>
                        <option value="<?=$rowOpen['major_cd'].$rowOpen['minor_cd']?>"><?=$rowOpen['cd_nm']?></option>
<?
    }
?>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="txtExplanation" id="txtExplanation" placeholder="어떤 영상인지 설명해 주세요" rows="5"></textarea>
                </div>
