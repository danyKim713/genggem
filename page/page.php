				...
				<article class="user-profile text-center mb-2">
					...
					<div class="d-flex text-center p-3 align-items-center justify-content-center">
						<div class="user-img box position-r">
							...
							<h5 class="fs-0 mt-2"><?=$rowM['닉네임']?></h5>
						</div>
						<div class="pl-3 w-100">
						 <h2 class="fs-05 mb-3  color-clip bg-gradient-primary fw-600"><?=$rowM['페이지이름']?></h2>
								...
								<ul class="user-info-list">
									...
									<li>
										...
										<p onclick="copyToAddress('#copyAddress')" class="address fs--1 d-inline">
											<span id="copyAddress" class="text-address"><?=isSecure()?"https":"http"?>://<?=$_SERVER['HTTP_HOST']?>/page/page.php?UID=<?=$rowM[UID]?></span>
											<button class="text-copy background-white">복사</button>
										</p>
										</div>
									</li>
									<li class="d-flex align-itmes-top mt-2">
										<label><i class="fas fa-bars color-8 fs--1 mt-1"></i> 가입한 클럽</label>
										<div class="d-table-cell">
...
?>
											...
											<li>
												<a href="channel_view.php?CID=<?=$rowC['CID']?>" title="바로가기" class="fs-005 color-6"><?=$rowC['채널이름']?> <?if($rowC['member_id']==$rowM['member_id']){?>(카페장)<?}?></a>
											</li>
...
?>
									...
									<li class="card border-none">
										<a href="page.php?UID=<?=$rowF['UID']?>" title="">
											<img class="card-img img-fluid" src="<?=phpThumb("/_UPLOAD/".$rowF['페이지프로필사진'], 300, 300, 0,"assets/images/user_img2.jpg")?>" width="300" alt="" /><br>
