<? 
    ...
    $strSaleChk = "";
    if ($rowLesson["sale_flg"] == "GS730YSA") {
        $strSaleChk = "checked";
    }

    $strShowChk = "";
    if ($rowLesson["show_flg"] == "AD001001") {
        $strShowChk = "checked";
    }
            ...
            <div class="row align-items-center justify-content-center">
                <div class="col-12 col-lg-10 mt-2">                 
					<div class="cart-totals-area mt-5">
                        <h5 class="title-- mt-4">레슨 수정</h5>

                        <form name="frm" id="frm" method="post" class="p-2">
						<input type="hidden" name="txtRecordNo" value="<?=$strRecordNo?>">
						<div class="shipping d-flex justify-content-between">
							<h5 style="width:30%;"><label for="레슨 제목" class="color-3 fw-400 mb-2">레슨 제목</label></h5>
							<div class="shipping-address" style="width:70%;">
								<input type="text" name="txtLessonTitle" id="txtLessonTitle" class="form-control" maxlength="30" placeholder="레슨상품명을 입력하세요. 예)원포인트레슨 30분" value="<?=$rowLesson["l_title"]?>">
							</div>
						</div>
						<div class="shipping d-flex justify-content-between">
							<h5 style="width:30%;"><label for="레슨 검색어" class="color-3 fw-400 mb-2">레슨 검색어</label></h5>
							<div class="shipping-address" style="width:70%;">
								<input type="text" name="txtTag" id="txtTag" class="form-control" maxlength="20" placeholder="검색어를 입력해 주세요. 쉼표(,)로 구분. 50자 이내" value="<?=$rowLesson["l_tag"]?>">
							</div>
						</div>
						<div class="shipping d-flex justify-content-between">
							<h5 style="width:30%;"><label for="레슨 가격" class="color-3 fw-400 mb-2">레슨 가격</label></h5>
							<div class="shipping-address" style="width:70%;">
								<input type="text" name="txtPrice" id="txtPrice" class="form-control" maxlength="10" placeholder="레슨 상품가격을 숫자로만 입력해 주세요." value="<?=$rowLesson["l_price"]?>">
							</div>
						</div>
						<div class="shipping d-flex justify-content-between">
...
?>
									<option <?=($rowLesson["쿠폰"] == $rowCoupon["c_id"]) ? "selected":"";?> value="<?=$rowCoupon["c_id"]?>"><?=$rowCoupon["쿠폰명"]?>[<?=number_format($rowCoupon["할인금액"])?>원] 적용</option>
<?
		}
?>
								</select>
								<p class="color-5 fw-200 mb-2 fs--1">
								<?if ($rowLesson["쿠폰사용여부"] == "AD005002") {?><span style="color:red">- 현재 레슨 상품은 쿠폰사용이 금지되었습니다. 설정이 되어도 적용이 되지 않습니다.</span> <br><?}?>
								<? if ($rowCouponLesson["cnt"] >= 2) { ?><span style="color:red">- 현재 쿠폰등록 상품 가능한 수량(최대 2개)이 초과 되었습니다.</span> <br><? } ?>
								- 쿠폰적용은 등록한 레슨 상품 중 최대 2개 까지만 적용가능합니다.<br>
								- 쿠폰적용시 할인된 금액은 레슨 판매금액 정산시 차감하여 정산됩니다.
								</p>
							</div>
						</div>
						<div class="shipping d-flex justify-content-between">
							...
							<div class="shipping-address" style="width:70%;">
								<input type="number" name="txtReward" id="txtReward" class="form-control" maxlength="7" placeholder="적립금을 숫자로만 입력해 주세요. 예) 200" value="<?=$rowLesson["마일리지"]?>">
						...
						<div class="shipping d-flex justify-content-between">
							<h5 style="width:30%;"><label for="레슨 카테고리" class="color-3 fw-400 mb-4">레슨 카테고리</label></h5>
							<div class="shipping-address" style="width:70%;">
								<select id="selCat" name="selCat" class="form-control mb-1" size="1"  onChange="javascript:getCatM()">
									...
									<? 
										...
										while ($row = mysqli_fetch_array($resultCategory)) {
											$strSelect = "";
											if ($rowLesson["cat_id"] == $row["cat_id"]) {
												$strSelect = " selected ";
											}
														echo "<option {$strSelect} value=\"{$row["cat_id"]}\">{$row["cat_nm"]}</option>";
										} 
									?>
								</select>
							</div>
						...
						<div class="shipping d-flex justify-content-between">
							...
							<div class="shipping-address" style="width:70%;">
								<select id="selCatM" name="selCatM" class="form-control mb-1" size="1">
									<option value="">--선택하세요--</option>
									<? 
										$query = "SELECT * FROM tbl_lesson_category WHERE parent_cat_id = {$rowLesson["cat_id"]} AND depth = 2 AND use_flg='AD005001' ORDER BY seq ";
										$resultCategory = db_query($query); 


										while ($row = mysqli_fetch_array($resultCategory)) {
											$strSelect = "";
											if ($rowLesson["catm_id"] == $row["cat_id"]) {
												$strSelect = " selected ";
											}
														echo "<option {$strSelect} value=\"{$row["cat_id"]}\">{$row["cat_nm"]}</option>";
										} 
						...
						<div class="shipping d-flex justify-content-between">
							<h5 style="width:30%;"><label for="레슨 기본지역 설정" class="color-3 fw-400 mb-4">레슨 기본지역 설정</label></h5>
							<div class="shipping-address" style="width:70%;">
...
while($rowA = db_fetch($resultA)){
	$strSelect = "";
	if ($rowLesson["클래스기본지역"] == $rowA["지점1"]) {
		$strSelect = " selected ";
	}
?>
									<option value="<?=$rowA['지점1']?>" <?=$strSelect?>><?=$rowA['지점1']?></option>
<?}?>
								</select>
							</div>
						</div>
						...
						<div class="shipping d-flex justify-content-between">
							<h5 style="width:30%;"><label for="우편번호" class="color-3 fw-400 mb-2">레슨 강의주소</label></h5>
							<div class="shipping-address" style="width:70%;">
								<input type="button" name="우편번호검색" id="우편번호검색" class="form-control" value="우편번호 검색" onClick="sample3_execDaumPostcode();">
								<input type="text" name="우편번호" id="우편번호" class="form-control" maxlength="10" placeholder="우편번호를 검색해 주세요." value="<?=$rowLesson['우편번호']?>" readonly>
								<input type="text" name="클래스주소" id="클래스주소" class="form-control" maxlength="60" placeholder="레슨 주소를 입력해 주세요."  value="<?=$rowLesson['클래스주소']?>" readonly>
								<input type="text" name="클래스상세주소" id="클래스상세주소" class="form-control" maxlength="20" placeholder="레슨 상세주소를 입력해 주세요." value="<?=$rowLesson['클래스상세주소']?>" >
							</div>
						</div>
						...
						<div class="shipping d-flex justify-content-between">
							<h5 style="width:30%;"><label for="레슨시작일" class="color-3 fw-400 mb-2">레슨 시작일</label></h5>
							<div class="shipping-address" style="width:70%;">
								<input type="text" name="클래스시작일" id="클래스시작일" class="form-control" maxlength="15" placeholder="예)6월 13일" value="<?=$rowLesson['클래스시작일']?>">
							</div>
						</div>

						<div class="shipping d-flex justify-content-between">
							<h5 style="width:30%;"><label for="레슨 난이도" class="color-3 fw-400 mb-4">레슨 난이도</label></h5>
							<div class="shipping-address" style="width:70%;">
							<select id="클래스난이도" name="클래스난이도" class="form-control mb-1" size="1">
									<option value="">--선택하세요--</option>
									<option value="쉬움" <?=$rowLesson['클래스난이도']=="쉬움"?"selected":""?>>쉬움</option>
									<option value="보통" <?=$rowLesson['클래스난이도']=="보통"?"selected":""?>>보통</option>
									<option value="조금높음" <?=$rowLesson['클래스난이도']=="조금높음"?"selected":""?>>조금높음</option>
									<option value="높음" <?=$rowLesson['클래스난이도']=="높음"?"selected":""?>>높음</option>
								</select>
							</div>
						</div>
						...
						<div class="shipping d-flex justify-content-between">
							...
							<div class="shipping-address" style="width:70%;">
								<input type="text" name="소요시간" id="ctime" class="form-control" maxlength="10" placeholder="예) 2시간(회)" value="<?=$rowLesson['소요시간']?>">
							</div>
						</div>
						...
						<div class="shipping d-flex justify-content-between">
							...
							<div class="shipping-address" style="width:70%;">
								<input type="text" name="수업인원" id="수업인원" class="form-control" maxlength="15" placeholder="예) 최대3명 or 1:1" value="<?=$rowLesson['수업인원']?>">
							</div>
						</div>

						<div class="shipping d-flex justify-content-between">
							<h5 style="width:100%;"><label for="레슨 상품설명" class="color-3 fw-400 mb-2">레슨 상품설명</label>
								<?=makeCKeditor("txtLessonGreetings", "txtLessonGreetings", $rowLesson['l_intro'], "100%", "250px")?><!-- <textarea name="txtLessonGreetings" id="txtLessonGreetings" class="form-control" placeholder="레슨상품 소개, 설명을 적어주세요" rows="18"><?=$rowLesson['l_intro']?></textarea> --> 
								</h5>
						...
						<div class="form-group row mx-0 mt-3">
							<div class="box col-2 p-0">
							 <label for="">* 사진1 (필수입력)</label>
								<div class="js-image-preview" style="background-image: url('/_UPLOAD/<?=$rowLesson["사진1"]?>'); height:140px;">
									<input type="file" id="Lessonp1" name="Lessonp1" class="image-upload" accept="image/*">
								</div>
							</div>
							<div class="box col-2 p-0">
								<label for="">사진2</label>
								<div class="js-image-preview" style="background-image: url('/_UPLOAD/<?=$rowLesson["사진2"]?>'); height:140px;">
									<input type="file" id="Lessonp2" name="Lessonp2" class="image-upload" accept="image/*">
								</div>
							</div>
							<div class="box col-2 p-0">
								<label for="">사진3</label>
								<div class="js-image-preview" style="background-image: url('/_UPLOAD/<?=$rowLesson["사진3"]?>'); height:140px;">
									<input type="file" id="Lessonp3" name="Lessonp3" class="image-upload" accept="image/*">
								</div>
							</div>
							<div class="box col-2 p-0">
								<label for="">사진4</label>
								<div class="js-image-preview" style="background-image: url('/_UPLOAD/<?=$rowLesson["사진4"]?>'); height:140px;">
									<input type="file" id="Lessonp4" name="Lessonp4" class="image-upload" accept="image/*">
								</div>
							</div>
							<div class="box col-2 p-0">
								<label for="">사진5</label>
								<div class="js-image-preview" style="background-image: url('/_UPLOAD/<?=$rowLesson["사진5"]?>'); height:140px;">
									<input type="file" id="Lessonp5" name="Lessonp5" class="image-upload" accept="image/*">
								</div>
							</div>
							<div class="box col-2 p-0">
								<label for="">사진6</label>
								<div class="js-image-preview" style="background-image: url('/_UPLOAD/<?=$rowLesson["사진6"]?>'); height:140px;">
									<input type="file" id="Lessonp6" name="Lessonp6" class="image-upload" accept="image/*">
								</div>
							</div>
						</div>
						<h5><label for="레슨난이도" class="color-3 fw-400 mb-4">(권장 사이즈 1,000 * 730)</label></h5>
						  ...
						  <label>레슨 상품판매</label>
							<ul class="list list-border background-white">
								<li class="p-3 d-flex align-items-center justify-content-between">
									...
									<div>
										<span class="small mr-1">중지</span> <label class="form-switch" for="chkSale">
											<input type="checkbox" id="chkSale" name="chkSale" <?=$rowLesson['sale_flg']=="GS730YSA"?"checked":""?> value="GS730YSA" /><i></i>
						...
						<div class="form-group">
						  <label>대표 레슨 설정 (대표레슨은 최대 2개까지 설정가능합니다.)</label>
							<ul class="list list-border background-white">
								<li class="p-3 d-flex align-items-center justify-content-between">
									<h4 class="fs-005 mb-0">설정여부</h4>
									<div>
										<span class="small mr-1">설정안함</span> <label class="form-switch" for="chkShow">
											<input type="checkbox" id="chkShow" name="chkShow" <?=$rowLesson['show_flg']=="AD001001"?"checked":""?>  value="AD001001" /><i></i>
						...
						<div class="mt-4">
							<a href="javascript:go_reg_ok();" class="btn alazea-btn w-100 mb-3">저장</a>
						</div>
						</form>
...
<script>
 ...
 $('.nav_category li[data-name="gnb-lesson"]').addClass('active');
    ...
    function go_reg_ok() {
        ...
        if (confirm("레슨을 수정하시겠습니까?")) {
            $.ajax({
                url: 'class_product_upload_modify_action.php',
                processData: false,
                contentType: false,
                data: formData,
                type: 'POST',
...
//return;
                    if (Data == "SUCCESS") {
                       ...
                       top.location.href = "class_product.php";
                        ...
                        alert('레슨 수정이 실패했습니다. 관리자에게 문의하세요.');
                    } else {
                        alert(Data);
                    }
                }
