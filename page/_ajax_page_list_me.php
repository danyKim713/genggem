...
$query = "select * from gf_page_article where member_id = '{$rowMember['member_id']}' order by pk_page_article DESC {$limit} ";
											...
											<div class="row m-0">
												<div class="col p-0 ">
													<div class="checkbox">
														...
														<label for="chk1" class="color-5 mb-0 fw-400" onClick="go_like('<?=$row['pk_page_article']?>', 'like_num_<?=$row['pk_page_article']?>');"><i class="far fa-thumbs-up fs-005 pr-1 color-5"></i>좋아요 <span class="color-primary" id="like_num_<?=$row['pk_page_article']?>"><?=(좋아요카운트($row['pk_page_article']))?></span></label>
													</div>
												</div>
												<div class="col p-0">
