...
function input_omit_check() {
    ...
    $('input, select').each(function () {
		...
		if (title !="" && title!=undefined ) {
			var msg = title +  " is needed.";
		}else{
			var msg = "mandatory data is required...";
		}
        if ($(this).attr("required" )=="required") {
            ...
            if (s == "") {
					if ($(this).hasClass('datepicker-from')) {
						s2 = $(this).parent().find(".datepicker-to").val()+"";
						s2 = s2.replace(/^\s*|\s*$/g, ''); // 좌우 공백 제거
						...
						s2 = $(this).parent().find(".datepicker-from").val();
						s2 = s2.replace(/^\s*|\s*$/g, ''); // 좌우 공백 제거
						if (s2 != "") {
							alert(msg);
							$(this).focus().select();
...
//입력항목 누락 체크 - 기간 입력창 고려안함
function input_omit_check_all() {
    var err_msg = "";
    $('.form-control').each(function () {
        ...
        if ($(this).attr("required" )=="required") {
            ...
            if (s == "") {
                    alert("mandatory data is required.");
                    $(this).focus().select();
                    err_msg = "err";
                    return false;
            }
...
function bind_comma_num(target) {
    var data,
        result;
...
function set_date_slash(str_date){
	str_date =  str_date.replace(/^\s*|\s*$/g, ''); 
	if (str_date != "" && str_date !=undefined )  {
  str_date  = str_date.substr(0,4)+"/"+str_date.substr(4,2)+"/"+str_date.substr(6,2);
	}
 return str_date;
 }

function set_date_dash(str_date){
